...
`
	...
	var err error
	for _, readme := range []string{"ReadMe.conf", "README.conf", "readme.conf"} {
	...
	if err != nil {
		return fmt.Errorf("read README configuration: %w", err)
	}

	htmlFile.WriteString(userReadMe)
	htmlFile.WriteString(footer)
	info("Writing HTML to ReadMe.html...")
	if err := os.WriteFile(dirPath+"assets/ReadMe.html", []byte(htmlFile.String()), 0o644); err != nil {
